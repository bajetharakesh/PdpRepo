...
public class TestPeaceTea extends Common {
    public static LinkedHashMap<String, String> PDPlinks = new LinkedHashMap<String, String>();

    @Parameters({"urlpeacetea"})
    @Test
    public void LaunchSite(String urlpeacetea) {
        help.launchUrl(urlpeacetea);
        help.checkPageLoad();
        ...
        if (allProduct != null)
        {
            for (int eachProduct = 0; eachProduct < allProduct.size(); eachProduct++) {
                ...
                if (((categoryForEachProduct.size() == 0))||(driver.getCurrentUrl().contains("peace"))) {
